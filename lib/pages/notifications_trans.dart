...
class Notifications_trans extends StatefulWidget {
  const Notifications_trans({Key? key}) : super(key: key);

  @override
  State<Notifications_trans> createState() => _Notifications_transState();
}

class _Notifications_transState extends State<Notifications_trans> {
  ...
  @override
  void initState() {
    gettingTransporterNotification();
    super.initState();
  }

  Future<void> gettingTransporterNotification()async {
    var result=await NotificationMethods().getTransporterNotify(sp['_id']);
    if (result.data['success']){
      var local_transporter=result.data['transporter'];
      setState(() {
        notices=local_transporter['notifications'];
      });
